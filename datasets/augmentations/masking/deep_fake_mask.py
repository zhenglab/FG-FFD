...
class ComponentsMasking(DeepFakeMasking):
    ...
    def compute_mask(self, img: np.ndarray, landmarks, idx: int) -> np.ndarray:
        ...
        idx2 = np.random.randint(0,8)
        path1 = self.paths[idx1]
        points1 = new_landmarks[path1]
        path2 = self.paths[idx2]
        points2 = new_landmarks[path2]
        mask1 = mask_from_points(img, points1)
        mask2 = mask_from_points(img, points2)
        masks = [mask1, mask2]
