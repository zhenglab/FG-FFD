...
def main():
    ...
    for ckpt in ckpts:
        if i ==len(ckpts)-1:
            ...
            print('eval_load', msg)
            methods = ['FFIW','Celeb-DF','DFDC','DFD','DFDCP','FaceForensics_c23']
            # methods = ['df-40-sadtalker','df-40-fomm','df-40-facedancer','df-40-mobileswap','df-40-simswap','df-40-inswap','df-40-uniface']
            for method in methods:
                ...
                # main test function
                test_perepoch(test_dataloader, model, args)
        i +=1
...
def test(dataloader, model, args):
    ...
    with torch.no_grad():
        for idx, datas in enumerate(tqdm(dataloader)):
            ...
            outputs= model(flow_fea, images, is_eval=True)
            # assert 0
            ...
            if isinstance(outputs, list):
                ...
                outputs = outputs
            outputs = torch.nn.functional.softmax(outputs, dim=2)[:,:,1]
...
def test_perepoch(dataloader, model, args):
    model.eval()
    test_label = 'test'
    y_outputs, y_labels, y_idxes = [], [], []
    y_outputs_f, y_labels_f, y_idxes_f = [], [], []
    with torch.no_grad():
        for idx, datas in enumerate(tqdm(dataloader)):
            images = datas['process_imgs']
            labels = datas['labels']
            ...
            y_labels.extend(labels)
            y_idxes.extend(idxes)
            labels_f = labels.unsqueeze(1).repeat(1, T).flatten(0,1)
            idxes_f = idxes.unsqueeze(1).repeat(1, T).flatten(0,1)
            outputs_f=torch.nn.functional.softmax(outputs_, dim=2)[:,:,1].flatten(0,1)  # [B, N*T, 2]
    ...
    for item in test_result_list:
        print(item)  
    auc, acc = video_evaluation.final_scores_withoutcsv(test_result_list, result_file=predict_file)
    ...
    for i, idx in enumerate(gather_y_idxes_f):
        ...
        if video_label >= 2:
            ...
            video_predict = 0.5
        test_result_list_f.append([video_name, video_label, video_predict])
    ...
    config_file_name = os.path.basename(config_file)
    copyfile(config_file, os.path.join(result_dir, config_file_name))
    aucf, accf = video_evaluation.final_scores_withoutcsv(test_result_list_f, result_file=predict_file)
    return auc, aucf
