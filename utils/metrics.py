...
def compute_metrics(model_outputs, labels):
    ...
    if len(model_outputs.shape) > 2:
        return compute_image_metrics(model_outputs, labels)
...
def compute_image_metrics(model_outputs, labels):
    """
    Compute the accuracy metrics.
    """
    real_probs = F.softmax(model_outputs, dim=2)[:,:, 0]
    real_probs = torch.mean(real_probs, dim=1)
    bin_preds = (real_probs <= 0.5).int()
    bin_labels = (labels != 0).int()
