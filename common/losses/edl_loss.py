    ...
    def edl_loss(self, func, y, alpha, annealing_coef, target):
        ...
        if self.with_avuloss:
            ...
            if self.disentangle:
                ...
                # acc_uncertain = -  torch.log(1 - uncertainty + self.eps)
                # inacc_certain = -   torch.log(uncertainty + self.eps)
            avu_loss = annealing_coef * acc_match * acc_uncertain + (1 - annealing_coef) * (1 - acc_match) * inacc_certain
            # avu_loss = (1 - annealing_coef) * acc_match * acc_uncertain + annealing_coef * (1 - acc_match) * inacc_certain
